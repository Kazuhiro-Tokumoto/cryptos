class what_is_this {
    modPow(base, exp, mod) {
        ...
        function windowModPow(base, exponent, modulus) {
            if (modulus === 1n)
                return 0n;
            if (exponent === 0n)
                return 1n; // 指数0のケースを明示
            // 負の基数に対応し、初期の剰余をとる
        ...
        if (mod % 2n === 0n) {
            console.log("Warning: Modulus is even. This implementation assumes an odd modulus for Montgomery reduction.");
    ...
    generatePrime(bits) {
        while (true) {
            ...
            n |= 1n;
            if (this.isProbablyPrime(n, 1) && this.isProbablyPrime(n, 19))
                return n;
        }
    }
    generateKey(bits = 2048) {
    ...
    encryptbigint(message, n, e) {
        ...
        function safeIvMaker(message, n) {
            // n のビット長を取得（例: 4096）
    ...
    decryptbigint({ iv, ciphertext }, n, e) {
    ...
    verifybigint(signature, message, n, e) {
        if (signature <= 0n ||
            signature >= n ||
            message <= 0n ||
            message >= n ||
    ...
    encrypt(message, n, e) {
        ...
        const ctBytes = this.bigIntToBytes(ciphertext, this.bigIntToBytes(N).length);
        return new Uint8Array([...ivBytes, ...ctBytes]);
    }
    decrypt(encrypted, n, e) {
        const N = this.bytesToBigInt(n);
        const E = this.bytesToBigInt(e);
