export class myjwt {
    ...
    //JWT作成
    //----------------------------------------------------------------------
    makeJWT(header, payload, signature) {
        const encode = (data) => {
    ...
    signschnorr(header, payload, privKey) {
    ...
    signecdsa(header, payload, privKey) {
        const headersorted = this.jsonsort(header);
        const payloadsorted = this.jsonsort(payload);
        const contacted = this.concat(headersorted, payloadsorted);
        const sorted = this.Sort(contacted);
    ...
    signHMAC(header, payload, secret) {
        const headersorted = this.jsonsort(header);
        const payloadsorted = this.jsonsort(payload);
        const contacted = this.concat(headersorted, payloadsorted);
        const sorted = this.Sort(contacted);
    ...
    selectAlgAndSign(alg, payload, key) {
        const header = { alg: alg, typ: "DLYAJWT" };
        switch (alg) {
            case "SchnorrP256":
                const sig = this.signschnorr(header, payload, key);
                return {
                    token: this.makeJWT(header, payload, this.concat(sig[0], sig[1], sig[2])),
    ...
    createJWT(alg, payload, key) {
        return { ...this.selectAlgAndSign(alg, payload, key) };
    }
    // ----------------------------------------------------------------------
    // JWT検証
    // ----------------------------------------------------------------------
    verifySchnorr(header, payload, signature, pubKey) {
        const headersorted = this.jsonsort(header);
        const payloadsorted = this.jsonsort(payload);
        const contacted = this.concat(headersorted, payloadsorted);
        const sorted = this.Sort(contacted);
    ...
    verifyECDSA(header, payload, signature, pubKey) {
        const headersorted = this.jsonsort(header);
        const payloadsorted = this.jsonsort(payload);
        const contacted = this.concat(headersorted, payloadsorted);
        const sorted = this.Sort(contacted);
    ...
    verifyHMAC(header, payload, signature, secret) {
        const headersorted = this.jsonsort(header);
        const payloadsorted = this.jsonsort(payload);
        const contacted = this.concat(headersorted, payloadsorted);
        const sorted = this.Sort(contacted);
    ...
    selectAlgAndVerify(heder, payload, signature, key) {
        ...
        switch (alg) {
            case "SchnorrP256":
                ...
                return this.verifySchnorr(heder, payload, [
            ...
            case "ECDSA_P256":
                ...
                return this.verifyECDSA(heder, payload, [signature.subarray(0, 32), signature.subarray(32, 64)], [key[0], key[1]]);
            case "HMAC_SHA256":
...
class p_256 {
    ...
    sign(message, privateKey) {
        const { r, s } = this.signtobigint(message, this.bigintToHex(this.bytesToBigInt(privateKey)));
        return [this.BigintToBytes(r), this.BigintToBytes(s)];
    }
    ...
    privateKeyToPublicKey(privateKeyHex) {
        const privKey = this.bytesToBigInt(privateKeyHex);
...
const jwtToken = jwt.createJWT("HMAC_SHA256", {
    "certificate": {
        "DYLA": [
            {
