class Dilithium2 {
  ...
  private rejUniformPoly(seed: Uint8Array, i: number, j: number): bigint[] {
    ...
    while (coeff < this.N) {
      ...
      for (let pos = 0; pos + 2 < buf.length && coeff < this.N; pos += 3) {
        const val =
          BigInt(buf[pos] | (buf[pos + 1] << 8) | (buf[pos + 2] << 16)) &
          0x7fffffn;
  ...
  private packT1(t1: bigint[][]): Uint8Array {
    ...
    for (let i = 0; i < this.K; i++) {
      for (let j = 0; j < this.N; j += 4) {
        const a = Number(t1[i][j]),
          b = Number(t1[i][j + 1]);
        const c = Number(t1[i][j + 2]),
          d = Number(t1[i][j + 3]);
        out[off++] = a & 0xff;
  ...
  keyGen(seed?: Uint8Array): { pk: Uint8Array; sk: Uint8Array } {
    ...
    const expanded = this.shake256(
      this.concat(seed, new Uint8Array([this.K, this.L])),
      128,
    );
  ...
  private polyMakeHint(
    z: bigint[],
    r: bigint[],
  ): { hint: number[]; count: number } {
  ...
  private expandMask(rhoPrime: Uint8Array, kappa: number): bigint[][] {
    ...
    for (let i = 0; i < this.L; i++) {
      ...
      for (let j = 0; j < this.N; j += 4) {
        const base = (j >> 2) * 9;
        const v0 =
          (buf[base] | (buf[base + 1] << 8) | (buf[base + 2] << 16)) & mask;
        const v1 =
          ((buf[base + 2] >> 2) |
            (buf[base + 3] << 6) |
            (buf[base + 4] << 14)) &
  ...
  private packW1(w1: bigint[][]): Uint8Array {
    ...
    for (let i = 0; i < this.K; i++) {
      for (let j = 0; j < this.N; j += 4) {
        const a = Number(w1[i][j]),
          b = Number(w1[i][j + 1]);
        const c = Number(w1[i][j + 2]),
          d = Number(w1[i][j + 3]);
  ...
  sign(skBytes: Uint8Array, msg: Uint8Array): Uint8Array | null {
    ...
    while (true) {
      ...
      if (
        !this.vecChkNorm(
          r0.map((p) => p.map((c) => this.mod(c, this.Q))),
          this.GAMMA2 - this.BETA,
        )
      )
        continue;
  ...
  verify(pkBytes: Uint8Array, msg: Uint8Array, sig: Uint8Array): boolean {
    ...
    const cTildeCheck = this.shake256(
      this.concat(mu, this.packW1(w1Prime)),
      32,
    );
...
$("#tab-bar").addEventListener("click", (e: MouseEvent) => {
  ...
  (document.getElementById(`panel-${tab}`) as HTMLElement).classList.add(
    "active",
  );
});
  ...
  requestAnimationFrame(() => {
    try {
      ...
      if (isHex64(skInput)) {
        ...
        sk = keypair.sk;
        showToast(
          "seedから鍵を生成しました。keygen タブで公開鍵を保存することをお勧めします",
        );
      } else {
...
$("#btn-sktopk").addEventListener("click", () => {
  ...
  requestAnimationFrame(() => {
    try {
      ...
      ($("#sktopk-timing") as HTMLElement).textContent =
        `${ms}ms  |  pk: ${pk.length}B`;
    } catch (err: any) {
